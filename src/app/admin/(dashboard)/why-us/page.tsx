...
import { upsertWhyChooseUsItem, deleteWhyChooseUsItem } from "@/actions/admin/content";
...
export default async function AdminWhyUsPage() {
  ...
  return (
    <div>
      <h1 className="font-display text-2xl font-bold">Pourquoi choisir TEPSON ART GROUP</h1>
      <p className="mt-1 text-sm text-text-muted">Les 3 blocs affichés en timeline sur la page d&apos;accueil.</p>
      ...
      <div className="mt-8 space-y-4">
        {items.map((item) => (
          <details key={item.id} className="rounded-xl border border-line bg-surface-alt p-4">
            ...
            <form action={upsertWhyChooseUsItem} className="mt-4 grid gap-3 sm:grid-cols-2">
            ...
            <div className="mt-3">
              <DeleteButton action={deleteWhyChooseUsItem} id={item.id} />
            </div>
          </details>
        ))}
      </div>
      ...
      <div className="mt-10 rounded-xl border border-dashed border-line p-4">
        <h2 className="font-display font-semibold">Ajouter un bloc</h2>
        <form action={upsertWhyChooseUsItem} className="mt-4 grid gap-3 sm:grid-cols-2">
